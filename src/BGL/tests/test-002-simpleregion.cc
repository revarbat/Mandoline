...
int main(int argc, char**argv)
{
    ...
    fout.open("output/test-002a-simpreg-union.svg", fstream::out | fstream::trunc);
    if (fout.good()) {
	svg.header(fout);
    ...
    fout.open("output/test-002b-simpreg-diff.svg", fstream::out | fstream::trunc);
    if (fout.good()) {
	svg.header(fout);
    ...
    fout.open("output/test-002c-simpreg-intsect.svg", fstream::out | fstream::trunc);
    if (fout.good()) {
	svg.header(fout);
